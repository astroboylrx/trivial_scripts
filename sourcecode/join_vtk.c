                ...
                for (j = 0; j < domain_3d[0][jg][0].Ny; j++)
                {
                    for (ig = 0; ig < NGrid_x; ig++)
                    {
                        ...
                        for (i = 0; i < domain_3d[0][0][ig].Nx; i++)
                        {
                            ...
                            if ((nread = fread(&fdat, sizeof(T), 1, fp)) != 1)
                                join_error("read_write_scalar error\n");

                            fwrite(&fdat, sizeof(T), 1, fp_out);
                        }
                    }
                }
            }
                ...
                for (j = 0; j < domain_3d[0][jg][0].Ny; j++)
                {
                    for (ig = 0; ig < NGrid_x; ig++)
                    {
                        ...
                        for (i = 0; i < domain_3d[0][0][ig].Nx; i++)
                        {
                            ...
                            if ((nread = fread(fvec, sizeof(T), 3, fp)) != 3)
                                join_error("read_write_vector error\n");

                            fwrite(fvec, sizeof(T), 3, fp_out);
                        }
                    }
                }
            }
    ...
    while (1)
    {
       ...
       VECTORS data */
        ...
        if (strcmp(type, "SCALARS") == 0)
        {
            ...
            if (need_float)
            {
                read_write_scalar<float>(fp_out);
            }
            if (need_double)
            {
                read_write_scalar<double>(fp_out);
            }
        }
        else if (strcmp(type, "VECTORS") == 0)
        {
            if (need_float)
            {
                read_write_vector<float>(fp_out);
            }
            if (need_double)
            {
                read_write_vector<double>(fp_out);
            }
        }
        else
