 ...
 *
 * COMPILE USING: g++ -Wall -W -o join_vtk join_vtk.c -lm
 *
                ...
                for (j = 0; j < domain_3d[0][jg][0].Ny; j++)
                {
                    for (ig = 0; ig < NGrid_x; ig++)
                    {
                        ...
                        for (i = 0; i < domain_3d[0][0][ig].Nx; i++)
                        {
                            ...
                            fdat = endian_reverse<double>(fdat);
                            fout = static_cast<float>(fdat);
                            fout = endian_reverse<float>(fout);
                            fwrite(&fout, sizeof(float), 1, fp_out);
                        }
                    }
                }
            }
                ...
                for (j = 0; j < domain_3d[0][jg][0].Ny; j++)
                {
                    for (ig = 0; ig < NGrid_x; ig++)
                    {
                        ...
                        for (i = 0; i < domain_3d[0][0][ig].Nx; i++)
                        {
                            ...
                            for (dim = 0; dim < 3; dim++) {
                                fvec[dim] = endian_reverse<double>(fvec[dim]);
                                fout[dim] = static_cast<float>(fvec[dim]);
                                fout[dim] = endian_reverse<float>(fout[dim]);
                            }
                            fwrite(fout, sizeof(float), 3, fp_out);
                        }
                    }
                }
            }
...
static void write_joined_vtk(const char *out_name)
{
    ...
    int retval;
    bool need_float = false, need_double = false;
    ...
    while (1)
    {
            ...
            for (j = 0; j < NGrid_y; j++)
            {
                for (i = 0; i < NGrid_x; i++)
                {
                    ...
                    if (i == 0 && j == 0 && k == 0)
                    {
                        ...
                        if (retval == EOF)
                        { /* Assuming no errors, we are done... */
                            fclose(fp_out);
                            return;
                        }
                        ...
                        {
                            fclose(fp_out);
                            join_error("Expected \"float\" or \"double\" format, found \"%s\"\n", format);
                        }
                    }
                    else
                    {
       ...
       VECTORS data */
        fprintf(fp_out, "%s %s float\n", type, variable); // convert to float
        if (strcmp(type, "SCALARS") == 0)
        {
